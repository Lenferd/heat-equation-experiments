...
def buildPlot3D(ax, title, u, rstride, cstride, usmin, usmax,
    ...
    X, Y = np.meshgrid(x, y)
    # Z Y X
    ax.plot_surface(X, Y, zmax, alpha=alpha, facecolors=cm.jet(norm(u[0,:,:])),
                    rstride=rstride, cstride=cstride, linewidth=linewidth,
                    antialiased=antialiased, shade=shade)
    ax.plot_surface(X, Y, zmin, alpha=alpha, facecolors=cm.jet(norm(u[-1,:,:])),
                    rstride=rstride, cstride=cstride, linewidth=linewidth,
                    antialiased=antialiased, shade=shade)
...
#z y x
    ...
    plt.savefig('finish.png', transparent=True)
    plt.show()
